    ...
    public void renderPlayerlist(int width, Scoreboard scoreboardIn, ScoreObjective scoreObjectiveIn)
    {
        ...
        for (int k4 = 0; k4 < l3; ++k4)
        {
            ...
            if (k4 < list.size())
            {
                ...
                /*if(s1.contains(mc.thePlayer.getName())) {
                	s1 = s1.replace(mc.thePlayer.getName(), "No RANK");
                }*/
                FakeName name = (FakeName) Client.instance.getModuleManager().getModuleByClass(FakeName.class);
            	try {
            		if(!ModuleManager.getModuleByName("FakeName").isEnabled() && name.Tab.getValue().booleanValue() && s1.contains(Minecraft.getMinecraft().thePlayer.getName())) {
            			s1 = s1.replace(Minecraft.getMinecraft().thePlayer.getName(), name.getFakeName());
                	}
            	} catch(Exception ex) {}
