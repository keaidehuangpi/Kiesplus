...
public class AStarCustomPathFinder {
    ...
    private Vec3[] flatCardinalDirections = new Vec3[]{new Vec3(1.0, 0.0, 0.0), new Vec3(-1.0, 0.0, 0.0), new Vec3(0.0, 0.0, 1.0), new Vec3(0.0, 0.0, -1.0)};
    ...
    public void compute(int loops, int depth) {
        ...
        for (int i = 0; i < loops; ++i) {
            ...
            for (Hub hub : new ArrayList<Hub>(this.hubsToWork)) {
                ...
                this.hubs.add(hub);
                Vec3[] arrvec3 = flatCardinalDirections;
                int n = flatCardinalDirections.length;
                for (int k = 0; k < n; ++k) {
                    Vec3 direction = arrvec3[k];
                    Vec3 loc = hub.getLoc().add(direction).floor();
                    if (AStarCustomPathFinder.checkPositionValidity(loc, false) && this.addHub(hub, loc, 0.0))
                        break block0;
                }
                Vec3 loc1 = hub.getLoc().addVector(0.0, 1.0, 0.0).floor();
                if (AStarCustomPathFinder.checkPositionValidity(loc1, false) && this.addHub(hub, loc1, 0.0) || AStarCustomPathFinder.checkPositionValidity(loc2 = hub.getLoc().addVector(0.0, -1.0, 0.0).floor(), false) && this.addHub(hub, loc2, 0.0))
                    break block0;
            }
        }
        if (this.nearest) {
